...
const commandName = (argv: ReadonlyArray<string>): { readonly command: string; readonly consumed: number } | ParseError => {
  const first = argv[0];
  ...
  if (first === "project" || first === "session" || first === "evidence" || first === "proposal") {
    const second = argv[1];
    if (second === undefined) return { _tag: "UsageFailure", reason: `${first} requires a subcommand` };
...
export const parseInvocation = (argv: ReadonlyArray<string>): ParsedInvocation | ParseError => {
  ...
  for (let index = named.consumed; index < argv.length; index += 1) {
    ...
    if (equal >= 0) {
      ...
      if (value.length === 0) return { _tag: "UsageFailure", reason: `option --${key} requires a value` };
      flags.set(key, value);
      continue;
    }
...
const requiredFlag = (invocation: ParsedInvocation, name: string): Effect.Effect<string, ParseError> => {
...
const readJsonFile = <S extends Schema.ConstraintDecoder<unknown>>(schema: S, path: string, name: string): Effect.Effect<S["Type"], ParseError> =>
  Effect.gen(function* () {
    const text = yield* Effect.tryPromise({
      ...
      catch: (error) => ({ _tag: "UsageFailure" as const, reason: `cannot read ${name}: ${error instanceof Error ? error.message : String(error)}` }),
    });
    return yield* Effect.try({
      try: () => Schema.decodeUnknownSync(schema, { onExcessProperty: "error" })(JSON.parse(text) as unknown),
      catch: (error) => ({ _tag: "UsageFailure" as const, reason: `invalid ${name}: ${error instanceof Error ? error.message : String(error)}` }),
    });
  });
...
const acceptedCreateResult = (result: ProjectCreateResult): Effect.Effect<ProjectCreateResult, CliFailure> => {
...
const observationRevision = (
  ...
  return value === undefined ? Effect.succeed(observation.eventRevision) : parseNatural(value, "--expected-revision").pipe(Effect.flatMap((revision) => parseWith(EventRevisionSchema, revision, "expected revision")));
};
...
const actorCommand = (
  ...
  Effect.gen(function* () {
    const revision = observation === undefined ? yield* parseNatural(flag(invocation, "expected-revision") ?? "0", "--expected-revision") : yield* observationRevision(invocation, observation);
    const id = yield* parseWith(CommandIdSchema, `cmd_${crypto.randomUUID()}`, "command id");
    return yield* client.dispatch(projectId, id, revision, command).pipe(Effect.flatMap(acceptedResult));
  });

export const buildCreateProjectRequest = (invocation: ParsedInvocation): Effect.Effect<CreateProjectRequest, ParseError> =>
  Effect.gen(function* () {
    const policyId = yield* parseWith(PolicyIdSchema, flag(invocation, "policy-id") ?? "pol_tracer_0001_v1", "policy id");
    const revision = yield* parseNatural(flag(invocation, "policy-revision") ?? "0", "--policy-revision");
    const maxAttempts = yield* parseNatural(flag(invocation, "max-attempts") ?? "2", "--max-attempts");
    const policy = yield* parseWith(PolicySchema, {
      _tag: "Policy",
      policyId,
      revision,
      requiredGates: ["gat_session_completed", "gat_candidate_present", "gat_scope_valid", "gat_check_passed", "gat_human_approved"],
      mergeCapability: "proposal.merge",
      maxAttempts,
    }, "policy");
    const command = yield* parseWith(CreateProjectSchema, { _tag: "CreateProject", policy }, "create command");
    return yield* parseWith(CreateProjectRequestSchema, {
...
const buildSubmitWork = (invocation: ParsedInvocation): Effect.Effect<ProjectCommand, ParseError> =>
  Effect.gen(function* () {
    const workId = yield* parseWith(WorkIdSchema, flag(invocation, "work-id") ?? `wrk_${crypto.randomUUID()}`, "Work id");
    const processId = yield* parseWith(WorkProcessIdSchema, flag(invocation, "work-process-id") ?? `wpr_${crypto.randomUUID()}`, "Work Process id");
    ...
    const writableScope = (flag(invocation, "writable-scope") ?? "src/greeting.ts").split(",").filter(Boolean);
    const title = flag(invocation, "title");
    const value: Record<string, unknown> = { _tag: "SubmitWork", workId, workProcessId: processId, objective, kind, writableScope, requiredCheck };
    if (title !== undefined) value.title = title;
    return yield* parseWith(SubmitWorkSchema, value, "submit command");
  });

const buildManagerSession = (invocation: ParsedInvocation): Effect.Effect<ProjectCommand, ParseError> =>
  Effect.gen(function* () {
    const sessionId = yield* parseWith(SessionIdSchema, flag(invocation, "session-id") ?? `ses_${crypto.randomUUID()}`, "Session id");
    const workId = yield* parseWith(WorkIdSchema, yield* requiredFlag(invocation, "work"), "Work id");
    const profileId = yield* parseWith(AgentProfileIdSchema, flag(invocation, "profile") ?? `prf_${crypto.randomUUID()}`, "Profile id");
    ...
    const deadline = yield* parseWith(TimestampSchema, flag(invocation, "deadline") ?? timestamp(30 * 60 * 1000), "deadline");
    const outputLimit = yield* parseNatural(flag(invocation, "output-limit") ?? `${10 * 1024 * 1024}`, "--output-limit");
    const toolBudget = yield* parseNatural(flag(invocation, "tool-budget") ?? "100", "--tool-budget");
    const resourceId = yield* parseWith(ResourceIdSchema, flag(invocation, "resource") ?? `res_${crypto.randomUUID()}`, "Resource id");
    const effectId = yield* parseWith(EffectIdSchema, flag(invocation, "effect") ?? `efx_${crypto.randomUUID()}`, "Effect id");
    return yield* parseWith(OpenManagerSessionSchema, { _tag: "OpenManagerSession", sessionId, workId, profileId, attempt, contextReference, deadline, outputLimit, toolBudget, resourceId, effectId }, "session start command");
  });

const buildCancelSession = (invocation: ParsedInvocation): Effect.Effect<ProjectCommand, ParseError> =>
  Effect.gen(function* () {
    const sessionId = yield* parseWith(SessionIdSchema, yield* requiredFlag(invocation, "session"), "Session id");
    const effectId = yield* parseWith(EffectIdSchema, flag(invocation, "effect") ?? `efx_${crypto.randomUUID()}`, "Effect id");
    const reason = flag(invocation, "reason") ?? "operator requested cancellation";
    return yield* parseWith(CancelSessionSchema, { _tag: "CancelSession", sessionId, effectId, reason }, "session cancel command");
  });

const buildApproval = (invocation: ParsedInvocation): Effect.Effect<ProjectCommand, ParseError> =>
  Effect.gen(function* () {
    const proposalId = yield* parseWith(ProposalIdSchema, yield* requiredFlag(invocation, "proposal"), "Proposal id");
    ...
    return yield* parseWith(ApproveProposalSchema, { _tag: "ApproveProposal", proposalId, evidence }, "approval command");
  });
...
const buildRejection = (invocation: ParsedInvocation): Effect.Effect<ProjectCommand, ParseError> =>
  Effect.gen(function* () {
    ...
    return yield* parseWith(RejectProposalSchema, { _tag: "RejectProposal", proposalId, reason }, "rejection command");
  });

const buildMerge = (invocation: ParsedInvocation): Effect.Effect<ProjectCommand, ParseError> =>
  Effect.gen(function* () {
    const mergeId = yield* parseWith(MergeIdSchema, flag(invocation, "merge-id") ?? `mrg_${crypto.randomUUID()}`, "Merge id");
    const proposalId = yield* parseWith(ProposalIdSchema, yield* requiredFlag(invocation, "proposal"), "Proposal id");
    const grantId = yield* parseWith(GrantIdSchema, yield* requiredFlag(invocation, "grant"), "Grant id");
    const candidateDigest = yield* parseWith(Sha256DigestSchema, yield* requiredFlag(invocation, "candidate"), "candidate digest");
    return yield* parseWith(MergeProposalSchema, { _tag: "MergeProposal", mergeId, proposalId, grantId, candidateDigest }, "merge command");
  });

const observationFor = (client: RemoteClient, projectId: ProjectId): Effect.Effect<ProjectObservation, CliFailure> => client.observe(projectId);

const projectIdFor = (invocation: ParsedInvocation): Effect.Effect<ProjectId, ParseError> =>
  positionalOrFlag(invocation, 0, "project").pipe(Effect.flatMap((value) => parseWith(ProjectIdSchema, value, "Project id")));
...
const why = (
  ...
  Effect.gen(function* () {
    ...
    const targetRevision = revisionValue === undefined
      ? undefined
      : yield* parseNatural(revisionValue, "--event-revision");
    const selected = targetRevision === undefined
      ? observation.history
  ...
  switch (invocation.command) {
    ...
    case "proposal show":
      return Effect.gen(function* () {
        ...
        const id = yield* requiredFlag(invocation, invocation.command === "evidence show" ? "evidence" : "proposal");
        const canonicalId = invocation.command === "evidence show"
        ...
        if (found === null) {
          return yield* Effect.fail({ _tag: "DomainFailure", reason: "canonical reference was not found" } as const);
        }
        return found;
      });
    case "approve":
    ...
    case "merge":
      return Effect.gen(function* () {
        const projectId = yield* projectIdFor(invocation);
        const observation = yield* observationFor(client, projectId);
        const command = invocation.command === "approve"
    ...
    case "attach":
      return Effect.gen(function* () {
        ...
        const request = yield* parseWith(AttachResolutionRequestSchema, { _tag: "AttachResolutionRequest", workId }, "attach request");
...
export const loadClientForInvocation = (
  ...
  Effect.gen(function* () {
    ...
    if (invocation.command === "mcp") return yield* Effect.fail({ _tag: "OperatorRequired", reason: "MCP uses a Session capability file" } as const);
