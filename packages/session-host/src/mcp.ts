...
export interface SessionMcpHandlers {
  ...
  readonly finalizeCandidate: (
    request: CandidateFinalizeRequest,
  ) => Promise<FrozenCandidate | unknown>;
}
...
export class SessionMcpServer {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      const status =
        error.failure._tag === "McpCapabilityInvalid" || error.failure._tag === "McpAuthorityDenied"
          ? 403
          : 400;
    ...
    switch (name) {
      ...
      case "session.start": {
        ...
        if (typeof workId !== "string")
          throw new McpAuthorityError({
            ...
            reason: "session.start requires workId",
          });
      ...
      case "candidate.finalize": {
        ...
        if (typeof reason !== "string" || reason.length === 0)
          throw new McpAuthorityError({
            _tag: "McpDecodeFailure",
            reason: "candidate.finalize requires reason",
          });
        return this.options.handlers.finalizeCandidate({
          sessionId: this.options.sessionId,
          reason,
        });
      }
...
export const parseSessionMcpArgs = (argv: readonly string[]): SessionId => {
  if (
    ...
    throw new Error("expected: work mcp --session <session-id>");
